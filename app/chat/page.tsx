...
const MOCK_RESULTS: Record<string, AssistantMessage> = {
  default: {
    ...
    restaurants: [
      { name: 'tuju', neighborhood: 'vila madalena', cuisine: 'contemporâneo', price: '$$$$', match: 96, description: 'iluminação baixa, mesas espaçadas. carta de vinhos curada por sommelier.', illustration: 'bowl' },
      { name: 'a casa do porco', neighborhood: 'centro', cuisine: 'brasileiro', price: '$$$', match: 91, description: 'ambiente íntimo no segundo andar, luz quente. reservar com antecedência.', illustration: 'glass' },
      { name: 'evvai', neighborhood: 'jardins', cuisine: 'italiano', price: '$$$$', match: 88, description: 'salão pequeno, atendimento atento. cozinha de autor.', illustration: 'plate' }
    ]
  },
  cafe: {
    type: 'restaurants',
    ...
    restaurants: [
      { name: 'coffee lab', neighborhood: 'vila madalena', cuisine: 'café especialidade', price: '$$', match: 94, description: 'wifi rápido, tomadas em todas as mesas. silêncio respeitado pela galera.', illustration: 'glass' },
      { name: 'nespresso boutique', neighborhood: 'itaim', cuisine: 'café', price: '$$$', match: 89, description: 'espaço amplo, luz natural, atendimento discreto. perfeito para reuniões.', illustration: 'bowl' },
      { name: 'madreselva', neighborhood: 'pinheiros', cuisine: 'café e bistrô', price: '$$', match: 85, description: 'jardim interno, mesas separadas, playlist baixa. bolo de cenoura excepcional.', illustration: 'plate' }
    ]
  },
  aniversario: {
    type: 'restaurants',
    ...
    restaurants: [
      { name: 'charco', neighborhood: 'pinheiros', cuisine: 'contemporâneo', price: '$$$', match: 97, description: 'mesa longa disponível, chef presente na sala. experiência gastronômica completa.', illustration: 'plate' },
      { name: 'chou', neighborhood: 'jardins', cuisine: 'francês', price: '$$$$', match: 92, description: 'salão intimista com reserva de ambiente privativo para grupos.', illustration: 'glass' },
      { name: 'banzeiro', neighborhood: 'itaim', cuisine: 'amazônico', price: '$$$', match: 87, description: 'cardápio único com ingredientes raros. experiência que o grupo não vai esquecer.', illustration: 'bowl' }
    ]
  }
}
...
function RestaurantCard({ restaurant, index }: { restaurant: Restaurant; index: number }) {
  ...
  return (
    <Link
      ...
      style={{
        display: 'block', background: '#fff', borderRadius: 8, overflow: 'hidden',
        border: '1px solid #e8e4dc', animation: `fadeIn 0.5s ease ${index * 0.12}s both`,
...
function UserAvatar({ name, size = 28 }: { name: string; size?: number }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: size * 0.36, color: '#fff', fontWeight: 600, lineHeight: 1 }}>
        {initials}
      </span>
    </div>
  )
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {user ? (
                ...
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#e8e4dc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ fontSize: 10, color: '#888' }}>df</span>
                </div>
              )}
              <span style={{ fontSize: 14, fontWeight: 500, color: '#333' }}>
                {user ? user.name.split(' ')[0].toLowerCase() : 'datafood'}
              </span>
            </div>
          ...
          {user ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: 'pointer', color: '#aaa', fontSize: 12, padding: '6px 4px', textAlign: 'left', whiteSpace: 'nowrap', transition: 'color 0.15s' }}
